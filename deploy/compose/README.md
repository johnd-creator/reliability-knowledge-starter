...
2. Set `COCKPIT_DB_PASSWORD`, `RELIABILITY_MART_DATABASE_URL`, and the three
   `*_COLLECTOR_API_BASE` values. `RELIABILITY_MART_DATABASE_URL` must point to
   the canonical Mart, not the legacy Cockpit database. When the populated
   Mart is the host's published Maximo collector database, use a DSN such as
   `postgresql+psycopg://<read-only-user>:<password>@host.docker.internal:5434/mxcollector`.
...
Managed mode supplies a separate Mart DSN to `cockpit-api` using the internal
`maximo-db:5432` service by default. Override
`RELIABILITY_MART_DATABASE_URL` only when the canonical Mart lives elsewhere;
never point it at `cockpit-db` or reuse the legacy `DATABASE_URL`.
...
| `cockpit-api` | serves legacy Cockpit routes plus read-only `/v1/reliability` queries against the separately configured Reliability Mart |
