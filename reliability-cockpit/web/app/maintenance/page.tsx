...
export default function MaintenancePage() {
  ...
  return <>
    ...
    <SectionCard className="data-section"><div className="section-heading"><div><p className="eyebrow">Reliability Mart / Registered Assets</p><h2>{page?.meta.total.toLocaleString("id-ID") ?? "—"} maintenance events</h2></div>{page && <span className="dataset-badge compact">Local Collector projection</span>}</div>
      {loading && <LoadingState />}{!loading && error && <ErrorState message={error} />}{!loading && !error && page && page.items.length === 0 && <EmptyState />}{!loading && !error && page && page.items.length > 0 && <><TableFrame minWidth={1100}><table><thead><tr><th>Work Order</th><th>Asset</th><th>Canonical event type</th><th>Status</th><th>Activity date</th><th>Actual finish</th><th>Downtime</th><th>Labor hours</th></tr></thead><tbody>{page.items.map((row) => <tr key={row.canonical_id}><td><Identifier value={row.work_order_id ?? row.id} /></td><td><Identifier value={row.equipment_id} /></td><td>{row.event_type ?? "UNKNOWN"}</td><td><StatusBadge value={row.status} /></td><td>{formatDate(row.actual_start ?? row.source_changed_at)}</td><td>{formatDate(row.actual_finish)}</td><td>{formatNumber(row.downtime_hours)} h</td><td>{formatNumber(row.labor_hours)} h</td></tr>)}</tbody></table></TableFrame><Pagination meta={page.meta} onChange={(offset) => setFilters((current) => ({ ...current, offset }))} /></>}</SectionCard>
  </>;
}
