...
export default function AssetsPage() {
  ...
  return <>
    <PageHeader eyebrow="Registered Asset projection" title="Asset Reliability" description="Registered Reliability Assets dari snapshot List of Assets, diperkaya konteks teknis lokal tanpa mengunduh seluruh dataset." actions={<span className="scope-chip">BSR / IP</span>} />
    ...
    <SectionCard className="data-section"><div className="section-heading"><div><p className="eyebrow">Reliability Mart / Registered Assets</p><h2>{page?.meta.total.toLocaleString("id-ID") ?? "—"} registered assets</h2></div>{page && <span className="dataset-badge compact">Current registry projection</span>}</div>
      {loading && <LoadingState />}{!loading && error && <ErrorState message={error} />}{!loading && !error && page && page.items.length === 0 && <EmptyState />}{!loading && !error && page && page.items.length > 0 && <><TableFrame minWidth={1120}><table><thead><tr><th>Asset</th><th>Description</th><th>Status</th><th>Location</th><th>Type</th><th>Plant</th><th>Unit</th><th>Last updated</th><th>Open</th></tr></thead><tbody>{page.items.map((asset) => <tr key={asset.canonical_id}><td><strong><Link className="asset-row-link" href={`/assets/${encodeURIComponent(asset.canonical_id)}`}>{asset.source_asset_number ?? <Identifier value={asset.canonical_id} />}</Link></strong><small className="table-subtext"><Identifier value={asset.canonical_id} /></small></td><td className="wide-cell">{asset.description ?? "—"}</td><td><StatusBadge value={asset.status} /></td><td>{asset.location_ref ?? "—"}</td><td>{asset.asset_type ?? "—"}</td><td>{asset.plant ?? "—"}</td><td>{asset.unit ?? "—"}</td><td>{formatDate(asset.source_updated_at)}</td><td><Link className="table-action" href={`/assets/${encodeURIComponent(asset.canonical_id)}`}>View Asset →</Link></td></tr>)}</tbody></table></TableFrame><Pagination meta={page.meta} onChange={changePage} /></>}</SectionCard>
  </>;
}
