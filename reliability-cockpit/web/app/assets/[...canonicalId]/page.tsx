...
function RecentHealth({ rows }: { rows: AssetHealthView[] }) {
  ...
  return <div className="asset-record-list">{rows.map((row) => <div className="asset-record" key={row.canonical_id}><div><strong>{row.source_record_id ?? "Asset Health assessment"}</strong><small>{row.function_description ?? row.description ?? "No description"}</small></div><StatusBadge value={row.lifecycle_status} /></div>)}</div>;
}
...
function HealthPanel({ page, onChange }: { page: Page<AssetHealthView>; onChange: (offset: number) => void }) {
  ...
  return <><TableFrame minWidth={880}><table><thead><tr><th>Record</th><th>Status</th><th>Revision</th><th>Description</th><th>Function</th><th>Created</th><th>Updated</th></tr></thead><tbody>{page.items.map((row) => <tr key={row.canonical_id}><td><strong>{row.source_record_id ?? "—"}</strong></td><td><StatusBadge value={row.lifecycle_status} /></td><td>{row.revision ?? "—"}</td><td className="wide-cell">{row.description ?? "—"}</td><td className="wide-cell">{row.function_description ?? "—"}</td><td>{formatDate(row.source_created_at)}</td><td>{formatDate(row.source_updated_at ?? row.status_changed_at)}</td></tr>)}</tbody></table></TableFrame><Pagination meta={page.meta} onChange={onChange} /></>;
}
