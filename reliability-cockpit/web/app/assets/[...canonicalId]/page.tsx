...
function RecentMaintenance({ rows }: { rows: MaintenanceEventView[] }) {
  if (rows.length === 0) return <EmptyState title="No recent maintenance in the current Mart data." />;
  return <TableFrame minWidth={560}><table><thead><tr><th>Work order</th><th>Event</th><th>Source Status</th><th>Date</th></tr></thead><tbody>{rows.map((row) => <tr key={row.canonical_id}><td><Identifier value={row.work_order_id ?? row.id} /></td><td>{row.event_type ?? "—"}</td><td><StatusBadge value={row.status} mode="raw-neutral" /></td><td>{formatDate(row.actual_start ?? row.source_changed_at)}</td></tr>)}</tbody></table></TableFrame>;
}

function RecentFmea({ rows }: { rows: FmeaView[] }) {
  if (rows.length === 0) return <EmptyState title="No recent FMEA assessment in the current Mart data." />;
...
function RecentHealth({ rows }: { rows: AssetHealthView[] }) {
  if (rows.length === 0) return <EmptyState title="No recent Asset Health assessment in the current Mart data." />;
...
function MaintenancePanel({ page, onChange }: { page: Page<MaintenanceEventView>; onChange: (offset: number) => void }) {
  ...
  return <><TableFrame minWidth={1050}><table><thead><tr><th>Work Order</th><th>Event / Work type</th><th>Source Status</th><th>Actual start</th><th>Actual finish</th><th>Duration</th><th>Downtime</th><th>Labor hours</th><th>Failure code</th></tr></thead><tbody>{page.items.map((row) => <tr key={row.canonical_id}><td><strong>{row.work_order_id ?? "—"}</strong></td><td>{row.event_type ?? "—"}</td><td><StatusBadge value={row.status} mode="raw-neutral" /></td><td>{formatDate(row.actual_start)}</td><td>{formatDate(row.actual_finish)}</td><td>{row.duration_hours == null ? "—" : `${row.duration_hours} h`}</td><td>{row.downtime_hours == null ? "—" : `${row.downtime_hours} h`}</td><td>{row.labor_hours == null ? "—" : `${row.labor_hours} h`}</td><td>{row.failure_code ?? "—"}</td></tr>)}</tbody></table></TableFrame><Pagination meta={page.meta} onChange={onChange} /></>;
}
